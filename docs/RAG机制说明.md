...
因此系统把 RAG 拆成：解析、结构化切片、检索前优化、混合检索、融合重排、多证据补召回（仅评测链路）、约束生成、评测反馈。
...
`.xls` 虽然在上传白名单内，但会被路由到只支持 `.xlsx`/`.xlsm` 的 openpyxl 解析器，老版 `.xls` 必然解析失败，请先另存为 `.xlsx` 再上传。
...
本项目的主切片策略是**整章独立成块**（`cut_by_heading`），而不是按固定字符数自然切分：
...
只有当单个章节超过 `MAX_CHAPTER_LEN=1200` 字符时，才由 `RecursiveCharacterTextSplitter` 兜底再切，此时才用到 `CHUNK_SIZE` / `CHUNK_OVERLAP`：

| 参数 | 默认值 | 作用 |
| --- | ---: | --- |
| `CHUNK_SIZE` | 350 | 超长章节兜底切分的每块目标字符数（主策略不使用）。 |
| `CHUNK_OVERLAP` | 80 | 超长章节兜底切分时相邻块重叠字符数（主策略不使用）。 |
...
`350 + 80` 只作用于极少数超长章节：块太小会拆散条款正文、例外条件和处理动作，块太大又容易形成“万能候选块”，适中长度配合重叠更利于证据可读性。
...
跨文档问题经常需要主证据和辅助证据。系统会针对 required citations 或问题中的多个来源分别补召回，避免最终上下文只来自一个文档。该机制**仅在 RAG 评测链路**（`evaluation_run_service.py`）启用，线上问答链路不做补召回。
