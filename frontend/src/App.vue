...
-->
<template>
  <LoginView v-if="authReady && !authUser && !authBlocked" @login-success="handleLoginSuccess" />
  <div v-else-if="authReady && authBlocked" class="auth-boot-screen">
    <div class="brand-mark"><span class="brand-letter">W</span></div>
    <strong>后端服务暂时不可用</strong>
    <span class="auth-blocked-hint">登录状态已保留，服务恢复后点下面的按钮继续，不用重新登录。</span>
    <el-button class="auth-blocked-retry" type="primary" @click="retryRestore">重试</el-button>
  </div>
...
// 有 token 但恢复失败、且不是登录失效（后端重启窗口/网络抖动/5xx）。
// 单独一态，避免把用户扔回登录页、逼他重新登录。
...
async function restoreSession() {
  ...
  try {
    ...
    // 只有「登录态真的失效」（401）才清 token 跳登录页。后端重启的那几秒、
    // 网络抖动、5xx 都会落到这里 —— 那时候把用户踢下线并清掉 token 是错的：
    // 服务恢复后他点一次重试就该回到原状态，不该被迫重新登录一遍。
...
/* 后端暂时不可用时的提示：说明"登录状态还在、点重试就行" */
.auth-boot-screen strong {
  color: var(--kb-text);
  font-size: 16px;
}
.auth-blocked-hint {
