...
<template>
  <main class="dashboard">
    ...
    <!-- RAG 评测中心：只展示可复现指标，避免把主观效果包装成虚高数据 -->
    <section class="panel evaluation-panel">
      <div class="panel-header evaluation-header">
        <div>
          <h3>RAG 评测中心</h3>
          <span class="panel-sub">52 题测试集 · 检索命中、章节命中与拒答能力</span>
        </div>
        <el-button
          type="primary"
          plain
      ...
      <div class="evaluation-body">
        <div class="evaluation-summary">
          <div class="eval-number">{{ evaluationCatalog?.case_count || 52 }}</div>
          <div class="eval-label">52 题测试集</div>
          <p>覆盖事实检索、跨文档综合、规则计算、矛盾检测和未覆盖问题拒答。</p>
        </div>
        <div class="eval-category-grid">
          <div
            v-for="cat in evaluationCategories"
            :key="cat.key"
            class="eval-category"
          >
            <span class="eval-cat-key">{{ cat.key }}</span>
            <span class="eval-cat-name">{{ cat.label }}</span>
            <strong>{{ cat.case_count }} 题</strong>
          </div>
        </div>
        <div class="evaluation-result">
          <div class="eval-result-title">真实指标</div>
          <template v-if="evaluationResult">
            <div class="eval-pass-rate">
              {{ evaluationResult.retrieval_pass_rate }}%
            </div>
            <div class="eval-pass-note">
              retrieval_pass_rate · {{ evaluationResult.retrieval_pass_count }}/{{ evaluationResult.retrieval_case_count }}
            </div>
            <p>{{ evaluationResult.note }}</p>
          </template>
          <template v-else>
            <div class="eval-placeholder">等待运行</div>
            <p>点击后基于当前知识库运行检索层评测，不默认触发模型生成成本。</p>
          </template>
        </div>
      </div>
    </section>
...
import { getEvaluationCatalog, getStatsSummary, getDocChunks, runEvaluation } from '../api/api.js'
...
const evaluationCategories = computed(() => {
  const categories = evaluationCatalog.value?.categories || {
    A: { label: '直接事实检索', case_count: 18 },
    B: { label: '跨文档综合', case_count: 8 },
    C: { label: '规则应用与计算', case_count: 8 },
    D: { label: '易错点与矛盾检测', case_count: 8 },
    E: { label: '未覆盖问题拒答', case_count: 10 },
  }
...
onMounted(async () => {
  try {
    ...
    evaluationCatalog.value = await getEvaluationCatalog()
    // 默认选中第一个知识库
...
async function handleRunEvaluation() {
  ...
  try {
    evaluationResult.value = await runEvaluation({
