        ...
        <div class="admin-panel evaluation-workbench">
          <div class="panel-head">
            ...
            <div class="eval-actions">
              ...
              <el-button size="small" plain :loading="datasetGenerating" @click="handleGenerateEvaluationDataset">
                AI辅助出题
              </el-button>
          ...
          <div class="evaluation-stage-grid">
            <section class="eval-stage-card">
              ...
              <div class="dataset-meta" v-if="selectedDataset">
                ...
                <el-tag v-if="selectedDataset.source_type === 'ai_generated'" size="small" effect="light">
                  {{ selectedDataset.review_status === 'human_reviewed' ? 'AI 生成，已人工审核' : '辅助出题草稿' }}
                </el-tag>
                <span>{{ selectedDataset.case_count }} 题</span>
              </div>
              <p class="panel-note">上传或 AI 辅助生成的题集会先进入草稿；审核后可运行，也仍可删除。AI 辅助题集不作为默认基准，系统内置题集作为可复现基准样例保留。</p>
            ...
            <section class="eval-stage-card">
              ...
              <p class="panel-note">full 模式会调用模型并产生 API 成本；建议先用 retrieval 定位切片和召回问题。</p>
          ...
          <div class="evaluation-layout">
            <div class="evaluation-run-list">
              ...
              >
                ...
                <button type="button" class="evaluation-run-select" @click="selectEvaluationRun(run)">
                  ...
                  <small>{{ evaluationRunStatusText(run.status) }} · {{ formatDuration(run.duration_ms, run.status) }} · {{ run.created_at }}</small>
                  <el-progress
            ...
            <div class="evaluation-detail">
              ...
              <el-table :data="evaluationCases" stripe class="evaluation-case-table" @row-click="openCaseDetail">
                ...
                <el-table-column label="Top-K" width="150">
                  <template #default="{ row }">
      ...
      <div v-if="activeCase" class="case-drawer">
        ...
        <dl>
          ...
          <dd>
            ...
            主证据 {{ formatMetricRate(activeCase.evidence_diagnostics?.primary_evidence_hit_rate) }}；
            辅助证据 {{ formatMetricRate(activeCase.evidence_diagnostics?.secondary_evidence_hit_rate) }}
...
const datasetGenerating = ref(false)
...
async function loadEvaluationRuns() {
  ...
  evaluationRuns.value = data.items || []
  ...
  if (!selectedEvaluationRun.value && evaluationRuns.value.length) {
    ...
    if (refreshed) selectedEvaluationRun.value = refreshed
    else {
...
async function handleRunEvaluation() {
  ...
  try {
    evaluationResult.value = await runEvaluation({ kb_id: kbId, mode: 'retrieval' })
...
async function handleCreateEvaluationRun() {
  ...
  if (evaluationMode.value === 'full') {
    try {
      await ElMessageBox.confirm('full 模式会调用模型并产生 API 成本，确认开始完整评测？', '运行完整评测', { type: 'warning' })
    } catch (error) {
      return
    }
  }
...
async function handleUploadEvaluationDataset(file) {
  try {
    ...
    ElMessage.error('题集上传失败，请检查文件内容')
  }
}

async function handleGenerateEvaluationDataset() {
  const kbId = selectedEvaluationKbId.value === 'all'
    ? Number((knowledge.value.knowledge_bases || [])[0]?.id || 0)
    : Number(selectedEvaluationKbId.value)
  if (!kbId) {
    ElMessage.warning('请先选择一个知识库')
    return
  }
  datasetGenerating.value = true
  try {
    const dataset = await generateEvaluationDataset({ kb_id: kbId, case_count: 12 })
    selectedDatasetVersion.value = dataset.version
    ElMessage.success('AI 辅助出题草稿已创建，请先审核证据与标准答案')
    await Promise.all([loadEvaluationDatasets(), loadSelectedDatasetDetail(), loadAudit()])
  } finally {
    datasetGenerating.value = false
  }
}
...
async function refreshSelectedEvaluationRun() {
  if (!selectedEvaluationRun.value?.id) return
  selectedEvaluationRun.value = await getEvaluationRun(selectedEvaluationRun.value.id)
  await loadEvaluationCases()
}
...
const evaluationMetrics = computed(() => {
  ...
  return [
    { label: 'Top-1', value: formatMetricRate(summary.top1_doc_hit_rate), note: '首位来源命中' },
    { label: 'Top-3', value: formatMetricRate(summary.top3_doc_hit_rate), note: '前三来源命中' },
    { label: 'Top-5', value: formatMetricRate(summary.top5_doc_hit_rate), note: '前五来源命中' },
    { label: '章节命中', value: formatMetricRate(summary.section_hit_rate), note: '章节/条款进入上下文' },
    ...
    { label: 'P95 耗时', value: `${summary.p95_latency_ms ?? 0}ms`, note: evaluationRunStatusText(selectedEvaluationRun.value?.status) },
  ]
})
...
.doc-row {
  min-height: 34px;
  display: grid;
  grid-template-columns: minmax(0, 1fr) 126px 64px;
  border-top: 1px solid var(--kb-border-light);
}
