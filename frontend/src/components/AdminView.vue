    ...
    <section class="admin-main">
        ...
        <div class="admin-panel">
          ...
          <el-table :data="enterprises" stripe class="enterprise-table">
            ...
            <el-table-column label="状态" width="168" align="center">
              <template #default="{ row }">
                <!-- disable-transitions：这两个标签的显隐由数据决定（转正后「体验」要消失），
                     而 el-tag 自带的缩放动画会在频繁刷新时卡住、让元素留在 DOM 里不摘掉，
                     表现为「点了转正、提示成功了，标签却还在」。状态标签不需要动画。 -->
                <el-tag :type="enterpriseStatusType(row.status)" effect="light" disable-transitions>
                  {{ enterpriseStatusText(row.status) }}
                </el-tag>
                <!-- 体验企业：从登录页免审核开通的，功能受限，等系统管理员转正 -->
                <el-tag v-if="row.is_trial" type="warning" effect="light" disable-transitions>体验</el-tag>
              </template>
            </el-table-column>
                  ...
                  <el-dropdown trigger="click">
                    ...
                    <template #dropdown>
                      <el-dropdown-menu>
                        ...
                        <el-dropdown-item v-if="row.is_trial" @click="promoteEnterpriseRow(row)">转为正式</el-dropdown-item>
      ...
      <section v-if="activeTab === 'users'" class="admin-section two-column">
        ...
        <form class="admin-panel create-user-card" @submit.prevent="submitUser">
          ...
          <div v-if="isTrialEnterprise" class="trial-limit-notice">
            体验账号不能创建子员工账号。需要这项功能，请让平台管理员把账号转为正式。
          </div>
          <label>
      ...
      <section v-if="activeTab === 'evaluation'" class="admin-section">
        <!-- 体验账号：评测整组接口后端都拒绝，所以这里干脆不发请求、直接给说明，
             免得页面一加载就弹一串 403 -->
        <div v-if="isTrialEnterprise" class="admin-panel trial-locked-panel">
          <h2>RAG 评测中心</h2>
          <p>体验账号暂不能使用 RAG 评测。</p>
          <p>需要这项功能，请让平台管理员把账号转为正式。</p>
        </div>
        <div v-else class="admin-panel evaluation-workbench">
          <div class="panel-head">
            <div>
              <h2>RAG 评测中心</h2>
...
// 体验账号（从登录页免审核开通的那种）。它的两个限制——不能建子员工、不能用评测——
// 由后端强制，这里只负责"页面能看到、点了有提示"。
// 数据来自 /api/enterprise/settings，所以页面上要等它加载完才准；
// 加载期间按"不是体验账号"渲染，最坏情况是提示晚一拍出现，不会误拦。
const isTrialEnterprise = computed(() => Boolean(enterpriseSettings.value?.enterprise?.is_trial))
...
const datasetFormatColumns = [
  ...
  { name: 'category', required: '', desc: '题型字母：A 单文档事实检索 / B 单文档细节定位与条款解释 / C 跨文档关联推理 / D 场景应用与规则计算 / E 边界条件与易错判断 / F 文档未覆盖与幻觉测试。只填首字母也行。' },
...
async function loadAll() {
  // 先取企业设置：体验账号的判断依赖它（is_trial），而评测接口对体验账号是 403，
  // 必须先知道身份再决定发不发评测请求，否则页面一打开就弹一串权限提示。
  if (isEnterpriseAdmin.value || isSystemAdmin.value) await loadEnterpriseSettings()
  const loaders = [loadOverview(), loadUsers(), loadKnowledge(), loadAudit()]
  if (!isTrialEnterprise.value) loaders.push(loadEvaluationDatasets(), loadEvaluationRuns())
...
async function promoteEnterpriseRow(row) {
  // 转正是在放权，所以加一道确认 —— 转完该企业就能建员工、用评测了
  const confirmed = await ElMessageBox.confirm(
    `把「${row.name}」转为正式账号？转正后该企业可以创建子员工、使用 RAG 评测。`,
    '转为正式账号',
    { confirmButtonText: '确认转正', cancelButtonText: '取消', type: 'warning' },
  )
    .then(() => true)
    .catch(() => false)
  if (!confirmed) return
  await promoteEnterprise(row.id)
  ElMessage.success('已转为正式账号')
  await loadEnterprises()
}
...
async function loadEvaluationCenter() {
  // 体验账号没有评测权限：后端整组 403，这里直接不发请求，
  // 页面上由 trial-locked-panel 说明原因
  if (isTrialEnterprise.value) return
...
async function submitUser() {
  // 体验账号不能建子员工：先在本地拦一下，给一句人话提示，
  // 而不是发出去等后端 403（后端那道门禁是给非正常客户端兜底的）。
  if (isTrialEnterprise.value) {
    ElMessage.warning('体验账号不能创建子员工账号，需要的话请让平台管理员把账号转为正式')
    return
  }
...
/* 体验账号的功能限制提示。后端是硬门禁，前端这里只负责把原因讲清楚，
   避免用户点了没反应、以为页面坏了。 */
.trial-limit-notice {
  margin-bottom: 12px;
  padding: 10px 12px;
...
.trial-locked-panel h2 {
  font-size: 18px;
}

.trial-locked-panel p {
  margin: 0;
  color: var(--kb-text-muted);
  font-size: 13.5px;
}
