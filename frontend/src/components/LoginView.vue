<template>
  <main class="login-light-shell">
    ...
    <section class="login-hero">
      ...
      <div class="login-kicker">企业知识智能入口</div>
      <h1>企业知识库智能问答平台</h1>
      <p>统一连接结构化混合检索、引用溯源、权限隔离与持续对话能力，让团队在可信边界内快速获得有来源、有上下文的答案。</p>
    ...
    <section class="login-panel" aria-label="登录">
      ...
      <div class="register-links">
        <button class="register-link" type="button" @click="openRegister('apply')">
          企业管理员注册申请
        </button>
        <button class="register-link" type="button" @click="openRegister('trial')">
          企业管理员体验账号注册
        </button>
      </div>
    ...
    <el-dialog
      ...
      :title="registerMode === 'trial' ? '企业管理员体验账号注册' : '企业管理员注册申请'"
      width="520px"
      class="enterprise-register-dialog"
    >
      <!-- 体验模式的边界说明：注册前就讲清楚，避免注册完发现功能点不动、以为是 bug。
           正式申请那条不需要，因为审核通过后功能是全的。 -->
      <div v-if="registerMode === 'trial'" class="trial-notice">
        <p class="trial-notice-title">体验账号说明</p>
        <p><b>注册后无需审核</b>，可直接登录使用。</p>
        <p><b>可以使用</b>：知识库建设、文档上传、智能问答、企业设置（配置模型 Key）、后台页面浏览。</p>
        <p><b>需要你先做</b>：在「企业设置」里配置你自己的模型 API Key —— 不配置就无法问答。</p>
        <p><b>暂不可用</b>：创建子员工账号、RAG 评测中心，这两项需要平台管理员开通。</p>
      </div>
      <el-form label-position="top" class="register-form">
        ...
        <el-form-item>
          ...
          <el-input
            ...
            :placeholder="registerMode === 'trial' ? '用于登录' : '审核通过后用于登录'"
          />
        </el-form-item>
        <el-form-item>
...
import { getCaptcha, login, registerEnterpriseAdmin, registerEnterpriseTrial } from '../api/api.js'
...
// 'apply' = 正式申请（提交后等系统管理员审核）
// 'trial' = 体验账号（免审核、注册即可用，但功能受限）
// 两种模式共用同一个弹窗和同一套字段，差别只有标题、说明区、按钮文字和成功提示。
const registerMode = ref('apply')
...
async function handleRegister() {
  ...
  const isTrial = registerMode.value === 'trial'
  ...
  try {
    ...
    // 两边表单字段完全一样，只是打的接口不同：一条进审核队列，一条直接可用
    if (isTrial) {
      await registerEnterpriseTrial(payload)
    } else {
      await registerEnterpriseAdmin(payload)
    }
    registerVisible.value = false
    ElMessage.success(
      isTrial
        ? '体验账号已创建，请用「企业代码 + 用户名 + 密码」登录。记得先在「企业设置」里配置你自己的模型 API Key，否则无法问答。'
        : '申请已提交，请等待系统管理员审核',
    )
  } finally {
...
/* 两个注册入口：正式申请（要审核）+ 体验账号（免审核、功能受限）。
   竖排而不是并排 —— 并排会把「体验账号」那行挤到换行，反而更乱。 */
.register-links {
  display: grid;
}
.register-links .register-link {
  margin-top: 6px;
}
/* 体验账号的边界说明。放在表单上方，注册前就能看到 ——
   免审核注册最怕用户不知道边界，注册完发现功能点不动会当成 bug。 */
.trial-notice {
  margin-bottom: 16px;
  padding: 12px 14px;
