...
<template>
  ...
  >
    ...
    >
      ...
      <el-table-column label="解析方式" width="124">
        <template #default="{ row }">
          <div class="parser-cell" :title="formatParserDetail(row)">
            <span>{{ formatParserName(row.parser_name) }}</span>
            <small v-if="row.parsed_chars">{{ row.parsed_chars }} 字</small>
          </div>
        </template>
      </el-table-column>
