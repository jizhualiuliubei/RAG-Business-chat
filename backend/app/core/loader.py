...
def _load_pdf(file_path: str, filename: str) -> list[Document]:
    ...
    为什么加超时：某些异常 PDF 的 pypdf 逐页提取极慢（实测 30s+），
    ...
    return _with_original_source(docs, filename)
...
def load_document(file_path: str, filename: str):
    ...
    if ext == ".txt":
        ...
        raise ValueError(
            f"不支持的文件类型: {ext}，仅支持 .txt / .pdf / .docx / .csv / .xlsx"
        )

    return _with_original_source(loader.load(), filename)
