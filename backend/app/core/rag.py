...
# "边界/例外/禁止/必须"这类词，实测是负收益 —— 这批企业制度文档每一页都有
# 一模一样的样板行（责任边界、例外规则、审计要求…），补进去的泛化词会逐字命中
# 样板行，把真正相关的条款挤下去（实测：采购审批那题从第 1 名掉到第 18 名）。
...
# BM25 的查询词被稀释，结果是每个子问题的条款排名都往后掉（实测 5/5 的多问题目
# 全部出现漏检，而同样内容单问时 6/7 全对）。
_DECOMPOSE_SYSTEM = (
...
def _chat_model(model_config: dict | None = None, temperature: float | None = None):
    ...
    同一个问题每次检索到的证据都不一样（实测出现过一次诚实拒答、一次编造制度规则）。
...
def _decompose_question(question: str, model_config: dict | None = None) -> list[str]:
    ...
    多问题目不拆的代价实测很明显：5 道多问题目全部出现漏检，而同样内容单问时 6/7 全对。
    """
...
def retrieve_contexts(
    ...
    #    逐条 embed 会让"多路改写"变成多次网络往返，本地实测每次约 0.4 秒。
    candidates = []
    ...
    # 条款挤出 Top-K。实测（出差交通那题，期望 TRV-02-001 高铁二等座）：rerank
    # 关掉时它排第 1，开着时掉出前 5，于是被答成"知识库没提供"。融合排序是两个
    ...
    # 原因：跨编码器也会栽在词汇鸿沟上。实测「我出差住的酒店比标准贵，能报吗？」这题，
    # 正确条款用原问题打分只有 0.366（低于阈值 0.40 被丢掉），换成含「住宿标准 超标 报销」
    # 的改写串后升到 0.855 并稳居第 1；整个候选池的分也从 0.1~0.4 抬到 0.5~0.86，
    # 顺带修掉了「最终只返回 2~3 条上下文」的问题。改写串的第一路永远是原问题，不丢原语义。
