...
3. 应用启动时：自动建表 + 首次启动时引导初始化系统企业下的两个知识库
4. 提供健康检查接口
...
from app import models  # 必须导入：触发 models/__init__.py，把所有 ORM 模型注册进去
...
PUBLIC_API_PATHS = {
    ...
    # 体验账号注册：同样要免登录可访问，才可能从登录页发起注册
    "/api/auth/enterprise-trial-register",
}
