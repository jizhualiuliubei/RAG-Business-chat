...
@router.post("/enterprises/{enterprise_id}/promote")
def promote_enterprise(
    enterprise_id: int,
    request: Request,
    db: Session = Depends(get_db),
    _admin=Depends(require_system_admin),
    ...
    与 approve 的区别：体验企业的状态本来就是 active（注册即可用），
    所以这里只翻 is_trial 标记，不碰状态。
    """
    try:
        enterprise = enterprise_service.promote_enterprise(
            db, enterprise_id, int(request.state.auth_user["sub"])
        )
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    if enterprise is None:
