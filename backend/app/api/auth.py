...
@router.post("/enterprise-trial-register")
def enterprise_trial_register(body: EnterpriseRegisterRequest, db: Session = Depends(get_db)):
    """体验账号注册：**免审核**，注册完即可登录，但功能受限。

    与 /enterprise-register 的唯一区别是结果——那条写 pending_review 等系统管理员审核，
    这条直接置 active 并把企业标成 is_trial。表单字段和校验逻辑两边完全共用，
    所以体验注册绕不过「企业代码已存在」「system 被保留」这些检查。
    """
    try:
        enterprise, user = enterprise_service.register_trial_enterprise_admin(db, body.model_dump())
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    return {
        "enterprise": enterprise_service.enterprise_to_dict(enterprise),
