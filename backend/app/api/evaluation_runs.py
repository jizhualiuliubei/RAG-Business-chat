...
from app.services.enterprise_evaluation_dataset import FULL_DATASET_VERSION

# require_full_access：体验账号（免审核注册）整组不能用评测，能看到页面但接口会 403。
# 它排在 require_admin 之后，所以系统管理员不受影响。
router = APIRouter(
    prefix="/api",
    tags=["RAG评测任务"],
    dependencies=[Depends(require_admin), Depends(require_full_access)],
)


class EvaluationRunCreate(BaseModel):
    kb_id: int | list[int]
    dataset_version: str = Field(default=FULL_DATASET_VERSION)
...
# 必须声明在 /evaluation-datasets/{dataset_version} 之前，否则会被路径参数吞掉。
@router.get("/evaluation-datasets/upload-template")
def download_upload_template():
    return Response(
        content=evaluation_dataset_service.build_upload_template().encode("utf-8-sig"),
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename*=UTF-8''{quote('题集上传模板.csv')}"},
    )
...
async def upload_evaluation_dataset(
    ...
    try:
        dataset = await evaluation_dataset_service.create_uploaded_dataset(
            db, file, request.state.auth_user
        )
    except ValueError as exc:
        # 解析不出结构化题目时直接拒绝，并把「缺什么、该怎么写」告诉用户，
        # 不再静默降级成一条草稿题。
        raise HTTPException(status_code=400, detail=str(exc))
