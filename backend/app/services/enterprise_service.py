...
def enterprise_to_dict(enterprise: Enterprise) -> dict:
    return {
        ...
        "is_trial": bool(enterprise.is_trial),
...
def is_trial_enterprise(db: Session, enterprise_id: int | None) -> bool:
    """该企业是不是「体验企业」（免审核注册、功能受限）。

    每次查库而不是读 token：体验账号转正后要立刻解除限制，不用重新登录。
    """
    enterprise = db.get(Enterprise, enterprise_id) if enterprise_id else None
    return bool(enterprise and enterprise.is_trial)
...
def _build_enterprise_and_admin(
    db: Session,
    payload: dict,
    *,
    ...
    is_trial: bool,
    user_status: str,
) -> tuple[Enterprise, User]:
    """注册企业 + 建管理员账号的公共部分。

    正式注册和体验注册只差三个值（企业状态 / 是否体验 / 用户状态），
    校验和落库逻辑必须完全一致 —— 抽出来共用，避免两条路各自演化后
    出现「体验注册能绕过企业代码校验」这类漏洞。
    """
...
def register_enterprise_admin(db: Session, payload: dict) -> tuple[Enterprise, User]:
    """正式注册：企业和管理员都置 pending_review，等系统管理员审核通过才能登录。"""
    return _build_enterprise_and_admin(
        db,
        payload,
...
def register_trial_enterprise_admin(db: Session, payload: dict) -> tuple[Enterprise, User]:
    """体验注册：**免审核**，企业和账号直接置 active，注册完就能登录。

    代价是企业被标记 is_trial=True，功能受限（不能建子员工、不能用评测），
    要由系统管理员在后台「转为正式」才解除。限制判断读数据库而不是写进 token，
    所以转正后立即生效，体验账号不需要重新登录。
    """
    return _build_enterprise_and_admin(
        db,
        payload,
...
def promote_enterprise(db: Session, enterprise_id: int, reviewer_id: int) -> Enterprise | None:
    """把体验企业「转为正式」：解除「不能建子员工、不能用评测」两个限制。

    只翻 is_trial 标记 —— 企业状态本来就是 active，所以体验账号不用重新登录，
    刷新页面即生效（限制判断每次都读数据库，不读 token）。
    """
    enterprise = db.get(Enterprise, enterprise_id)
    if not enterprise:
        return None
    if not enterprise.is_trial:
        # 校验放在提交之前：先确认它确实是体验企业，再改标记
        raise ValueError("该企业不是体验账号")
    enterprise.is_trial = False
    enterprise.reviewed_by = reviewer_id
    enterprise.reviewed_at = datetime.now()
    db.commit()
    db.refresh(enterprise)
...
def require_enterprise_deepseek_key(db: Session, enterprise_id: int) -> dict:
    ...
    if item is None or not item.deepseek_api_key_encrypted:
        if enterprise is not None and enterprise.is_trial:
            # 体验账号自己就是管理员，"请联系企业管理员"对它没有意义
            raise ValueError(
                "体验账号需要用你自己的模型 Key：进入「企业设置」填入 DeepSeek API Key 后即可问答"
            )
...
def require_enterprise_siliconflow_key(db: Session, enterprise_id: int) -> dict:
    ...
    if item is None or not item.siliconflow_api_key_encrypted:
        if enterprise is not None and enterprise.is_trial:
            raise ValueError(
                "体验账号需要用你自己的模型 Key：进入「企业设置」填入 SiliconFlow API Key 后即可问答"
            )
