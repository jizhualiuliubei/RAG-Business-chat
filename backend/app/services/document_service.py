...
class VectorCleanupError(ValueError):
    """向量库未清理干净时抛出，避免 DB 记录先删造成幽灵向量。"""
...
def create_document(db, file: UploadFile, kb_id: int, enterprise_id: int | None = None) -> dict:
    ...
    # 体验企业有文档数量上限：它对公网开放，不封顶会被批量注册的脚本把磁盘占满
    if enterprise_id is not None and enterprise_service.is_trial_enterprise(db, enterprise_id):
        owned = db.execute(
            select(func.count())
            .select_from(Document)
            .where(Document.enterprise_id == enterprise_id)
        ).scalar_one()
        if owned >= TRIAL_MAX_DOCUMENTS:
            raise ValueError(
                f"体验账号最多上传 {TRIAL_MAX_DOCUMENTS} 份文档，"
                "需要更多请让平台管理员把账号转为正式"
            )
...
def delete_document(db, doc_id: int, enterprise_id: int | None = None) -> bool:
    ...
    # 1. 先强一致删除 Milvus 里该文档的全部向量。失败则保留 DB 记录供重试。
    _delete_vectors_strict([doc_id], enterprise_id=enterprise_id)
...
def delete_documents(db, doc_ids: list[int], enterprise_id: int | None = None) -> int:
    ...
    _delete_vectors_strict([doc.id for doc in docs], enterprise_id=enterprise_id)
...
def _delete_vectors_strict(doc_ids: list[int], enterprise_id: int | None = None) -> None:
    """同步删除并验证向量已清空；未清空则阻止 DB 删除。"""
    ...
    try:
        ...
        raise VectorCleanupError(f"向量库清理未完成，请稍后重试：{exc}") from exc

    count_fn = getattr(milvus_store, "count_by_doc_ids", None)
    if not callable(count_fn):
        return
    try:
        try:
            remaining = count_fn(ids, enterprise_id=enterprise_id)
        except TypeError as exc:
            if "enterprise_id" not in str(exc):
                raise
            remaining = count_fn(ids)
    except Exception as exc:
        raise VectorCleanupError(f"向量库删除后校验失败，请稍后重试：{exc}") from exc
    if int(remaining or 0) > 0:
        raise VectorCleanupError(f"向量库清理未完成，仍残留 {remaining} 个向量片段，请稍后重试")
...
def process_document(doc_id: int, stored_path: str, filename: str, enterprise_id: int | None = None) -> None:
    ...
    try:
        ...
        # 第3步：读取文档所属企业和知识库，用于按企业路由 Key 与向量隔离。
        ...
        parser_name, parser_version, parsed_chars = _parser_summary(docs)

        # 第6步：更新文档状态 + 记录切分块数和解析方式
        _update_doc_status(
            doc_id,
            status="done",
...
def _parser_summary(docs: list) -> tuple[str, str, int]:
    """汇总本次解析使用的解析器与字符数。"""
    parser_names = []
    parser_versions = []
    parsed_chars = 0
    for doc in docs:
        ...
        name = metadata.get("parser_name")
        version = metadata.get("parser_version")
        if name and name not in parser_names:
            parser_names.append(str(name))
        if version and version not in parser_versions:
            parser_versions.append(str(version))
        parsed_chars += len((getattr(doc, "page_content", "") or "").strip())
    return (
        "+".join(parser_names)[:80] if parser_names else "legacy-loader",
        "+".join(parser_versions)[:80],
        parsed_chars,
    )
