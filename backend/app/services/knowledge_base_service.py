...
from app.config import TRIAL_MAX_KNOWLEDGE_BASES, UPLOAD_DIR
...
def create_knowledge_base(db: Session, name: str, enterprise_id: int | None = None) -> KnowledgeBase:
    ...
    # 体验企业有数量上限：它对公网开放，不封顶会被批量注册的脚本把磁盘占满
    if enterprise_service.is_trial_enterprise(db, enterprise_id):
        owned = db.execute(
            select(func.count())
            .select_from(KnowledgeBase)
            .where(KnowledgeBase.enterprise_id == enterprise_id)
        ).scalar_one()
        if owned >= TRIAL_MAX_KNOWLEDGE_BASES:
            raise ValueError(
                f"体验账号最多创建 {TRIAL_MAX_KNOWLEDGE_BASES} 个知识库，"
                "需要更多请让平台管理员把账号转为正式"
            )
    # 检查重名：知识库名应该是唯一的
