...
class EvaluationCaseResult(Base):
    ...
    # 补召回之前的真实检索表现。NULL 表示该次评测没有统计（旧数据），不要当成 0。
    retrieval_top1_hit: Mapped[int | None] = mapped_column(Integer, nullable=True, default=None)
    retrieval_top3_hit: Mapped[int | None] = mapped_column(Integer, nullable=True, default=None)
    retrieval_top5_hit: Mapped[int | None] = mapped_column(Integer, nullable=True, default=None)
    retrieval_section_hit: Mapped[int | None] = mapped_column(Integer, nullable=True, default=None)
    # 端到端判定（不补召回、不做过度拒答修复）= 用户真实路径的通过情况。
    # NULL 表示该次评测没有统计（旧数据或 retrieval 模式），不要当成不通过。
    e2e_passed: Mapped[int | None] = mapped_column(Integer, nullable=True, default=None)
    e2e_answer_coverage: Mapped[int | None] = mapped_column(Integer, nullable=True, default=None)
    e2e_refusal_passed: Mapped[int | None] = mapped_column(Integer, nullable=True, default=None)
    e2e_failure_reason: Mapped[str | None] = mapped_column(String(255), nullable=True, default=None)
    ...
    ragas_scores_json: Mapped[str] = mapped_column(Text, nullable=False, default="{}")
    ragas_passed: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    ragas_skip_reason: Mapped[str] = mapped_column(String(255), nullable=False, default="")
    ragas_warning_reason: Mapped[str] = mapped_column(String(255), nullable=False, default="")
