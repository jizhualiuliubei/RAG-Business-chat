...
用法（必须在 backend 目录下运行，保证 app 可导入）：
    D:/development/miniconda3/envs/langchain1.2/python.exe scripts/rebuild_kb.py
...
注意：
- 只遍历 MySQL 的 document 表（26 条），不处理 uploads 里的孤儿文件（SQLite 遗留）。
- 每个文档先删 Milvus 旧 chunk 再写入（幂等，重复执行安全）。
- 解析为空文本的文档（如 doc_64 扫描版 PDF）置 failed，不写入向量。
"""
import sys
import time
