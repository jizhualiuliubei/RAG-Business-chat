...
<a href="http://118.31.45.253/"><img height="44" alt="点击进入在线体验" src="https://img.shields.io/badge/%E7%82%B9%E5%87%BB%E8%BF%9B%E5%85%A5%E5%9C%A8%E7%BA%BF%E4%BD%93%E9%AA%8C-2454d6?style=for-the-badge" /></a>　用演示账号登录，<b>直接在线提问</b><br />
...
| 企业审核、启用禁用与软删除，治理动作全部写入审计日志 | 员工账号的创建、编辑、启停与删除，全部按企业隔离 |
...
| **RAG 评测中心** | 内置 360 题企业语料题集，支持检索评测、主评测、RAGAS 诊断、主评测+诊断四种模式，出逐题失败原因并可导出报告，任务与报告按企业隔离。 |
...
| **真实检索 Top-5 文档命中率**（不补召回，用户真实路径） | **99.11%** |
| 证据 Top-5 可用率（补召回后，诊断上限） | **100.0%** |
...
| 企业代码 | 用户名 | 密码 | 角色 |
| --- | --- | --- | --- |
| `system` | `employee` | `employee123` | 员工演示账号，只能进入问答工作台 |

> 这个账号只能看问答，看不到管理后台——想看后台用**方式二**。公开演示环境的账号与数据仅用于功能体验。

#### 方式二：注册体验账号，免审核进管理后台

如果你想看**管理后台**（知识库治理、模型配置、评测中心），又不想等审核，用这个：

1. 登录页底部点「**企业管理员体验账号注册**」（旁边还有一条「企业管理员注册申请」，那个需要审核）
2. 填写企业名称、企业代码、管理员用户名和密码——和正式申请是同一张表单
3. **提交后无需审核**，直接用「企业代码 + 用户名 + 密码」登录，进入管理中心
4. 进「**企业设置**」填入你自己的模型 API Key —— **不配就无法问答**。用的是你自己的额度，不消耗演示环境的
5. 之后就能建知识库、传文档、提问，流程和正式企业一样

**体验账号的边界**（注册弹窗里也会写明）：

| | |
| --- | --- |
| 可以使用 | 知识库建设、文档上传、智能问答、企业设置、后台各页面浏览 |
| 需要你先做 | 在「企业设置」里配置你自己的模型 API Key |
| 暂不可用 | 创建子员工账号、RAG 评测中心（需平台管理员开通） |
| 数量上限 | 知识库 3 个、文档 20 份 |

#### 方式三：注册自己的企业，走一遍完整审核流程

如果你想走**正式注册**这条路（提交申请 → 平台审核 → 开通全部功能），按下面几步来：

1. **提交注册申请**：登录页底部点「企业管理员注册申请」，在弹出的表单里填写对应信息。

2. **等待审核**：提交后企业和管理员账号都处于**待审核**状态，此时还不能登录。平台管理员在「企业管理」里审核通过后，企业才会启用。

3. **登录管理中心**：用「企业代码 + 管理员用户名 + 密码」登录。企业管理员登录后进入管理中心，而不是问答工作台。

4. **配置模型 Key**：进「企业设置」，填入对话模型与嵌入模型的 Key，点「检验连接」确认可用。**没有配 Key 无法问答**。
...
7. **（可选）给同事开账号**：在「用户管理」里创建员工账号，员工登录后只能看到问答工作台。

> **方式三**的注册需要平台管理员**审核**，不会自动通过；想跳过审核用**方式二**。

## 技术栈

**后端** FastAPI · LangChain 1.2 · SQLAlchemy · Pydantic · MCP（工具接入协议）
...
**存储** Zilliz / Milvus（向量）· SQLite / MySQL（关系）
